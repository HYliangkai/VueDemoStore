...
<div class="body m-t-5 ">
    ...
    <swiper-slide v-for="item in swiper" :key="item._id" >
        <img :src="item.swiperurl" style="width:90% ;border-radius:0.2rem"  alt="为加载">
    </swiper-slide>
    <div class="swiper-pagination" slot="pagination"></div>
    </swiper>
    </div>
    <div class="card p-x-3  d-flex jc-around  p-t-4 wrap " style="width:100%;">
    <card :cardinfo="item" v-for="(item,index) in items" :key="index" class="m-2" style="border-radius: 2px;"></card>
    </div>
</div>
</template>

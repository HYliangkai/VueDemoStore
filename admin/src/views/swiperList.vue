<template>
<div>
    <h1 style="font-size: 2rem">轮播图展示</h1>
    <div>
    <el-table :data="model" stripe style="width: 100%" class="f-center">
        <el-table-column prop="_id" label=" id " width="width">
        </el-table-column>
        <el-table-column label=" 轮播图 " width="width">
        <template slot-scope="scope">
            <img :src="scope.row.swiperurl" style="height: 6rem" />
        </template>
        </el-table-column>
...
methods: {
    async fetchdata() {
    const rep = await this.$http.get("rest/swiper");
    this.model = rep.data;
    },
    deleterow(e) {
    this.$confirm("此操作将永久删除该轮播图, 是否继续?", "提示", {
        ...
        const rep = await this.$http.delete(`rest/swiper/${e._id}`);
        this.fetchdata();
        this.$message({
            type: "success",
            message: "删除成功!",
    ...
    editrow(e) {
    this.$router.push(`/swiper?id=${e._id}`);
    },
},
created() {
    this.fetchdata();
